...
| 05 | `05-rank-bar-and-eval-delta-view-state.md` | `RankBarViewState`（`EvalDeltaDisplayState` は上流待ちで**保留**） | 上部順位バー・評価の増減表示 |
...
4. 各仕様書の「未確定な演出との境界」で示した演出詳細が決まったら、対応するViewコンポーネントの仕様書（`../01-network-client.md` 等と同格の番号付きファイル）を別途起こす。この`value-objects/`側は骨格の変更（区分の追加等）があった場合のみ更新する。
