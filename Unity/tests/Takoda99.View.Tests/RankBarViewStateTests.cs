...
namespace Takoda99.View.Tests
{
    public class RankBarViewStateTests
    {
        ...
        {
            var state = RankBarViewState.From(evalNormalized: 0.25, aliveCount: 60, maxStores: 99, stormThresholdPct: 0.2);
            Assert.Equal(0.25f, state.SelfPositionRatio, 5);

            var updated = RankBarViewState.From(evalNormalized: 0.9, aliveCount: 40, maxStores: 99, stormThresholdPct: 0.2);
        ...
        {
            var state = RankBarViewState.From(evalNormalized: 0.5, aliveCount: 0, maxStores: 0, stormThresholdPct: 0.2);
        ...
        {
            var state = RankBarViewState.From(evalNormalized: 0.5, aliveCount: 33, maxStores: 99, stormThresholdPct: 0.2);
        ...
        {
            var state = RankBarViewState.From(evalNormalized: 0.5, aliveCount: 50, maxStores: 99, stormThresholdPct: 0.2);
